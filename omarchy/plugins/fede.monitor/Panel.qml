...
Panel {
  ...
  function toggleDisplay(name, enabled) {
    ...
    // The internal laptop panel has a dedicated Omarchy helper that persists
    // the choice: it writes the internal-monitor-disable toggle flag and
    // reloads. Without that flag, the clamshell reconciler (which runs every
    // few seconds while docked) turns the laptop panel straight back on, and
    // the next `hyprctl reload` re-applies monitors.lua's catch-all rule to
    // every output. A bare `hyprctl keyword ...,disable` does neither, so the
    // laptop screen kept reappearing and stealing focus — which also made the
    // focused-monitor scale look like it had reset.
    if (name === root.internalMonitor) {
      actionProc.command = ["omarchy-hyprland-monitor-internal", enabled ? "off" : "on"]
    } else {
