...
Panel {
  ...
  // ALSA cards can expose separate output "profiles" (Speakers, HDMI, HDMI 2,
  // etc.) of which only one is active at a time — PipeWire only creates a
  // sink for whichever profile is currently selected. These are discovered
  // dynamically (not hardcoded to one machine's PCI address) by parsing
  // `pactl -f json list cards`, and folded into the OUTPUT list as selectable
  // rows alongside any profile-independent devices (Bluetooth, USB) that
  // PipeWire already lists regardless of card profile.
  property string pendingDefaultSinkName: ""
  property int pendingDefaultSinkRetries: 0

  // Every currently-available output profile, across every real ALSA sound
  // card on this machine. Populated by updateOutputProfileOptions() below.
  // Each entry: { label, glyph, profile, sinkSuffix, sinkPrefix, cardName }.
  property var outputProfileOptions: []

  // cardName -> currently active profile id, for the whole machine.
  property var activeCardProfiles: ({})

  function updateOutputProfileOptions(raw) {
    var cards
    try { cards = JSON.parse(raw) } catch (e) { return }
    ...
    for (var c = 0; c < cards.length; c++) {
      var card = cards[c]
      if (!card || card.driver !== "alsa" || !card.name) continue
      activeMap[card.name] = card.active_profile || ""

      // pactl's json profiles field has shown up as both an object keyed by
      // profile id and as an array of profile objects across versions —
      // normalize both into a flat list.
      var entries = []
      var profiles = card.profiles
      if (Array.isArray(profiles)) {
        for (var p = 0; p < profiles.length; p++) {
          var pr = profiles[p] || {}
          entries.push({ id: pr.name || pr.id, description: pr.description, available: pr.available, sinks: pr.sinks })
        }
      } else if (profiles && typeof profiles === "object") {
        for (var key in profiles) {
          var val = profiles[key] || {}
          entries.push({ id: key, description: val.description, available: val.available, sinks: val.sinks })
        }
      }

      // Group by the pure output identifier (an id can be a solo
      // "output:X" or a duplex "output:X+input:Y" — prefer the solo form
      // when both exist for the same physical output).
      var byOutputId = {}
      for (var e = 0; e < entries.length; e++) {
        var entry = entries[e]
        if (!entry.id || entry.id.indexOf("output:") !== 0) continue
        if (entry.available === false) continue
        if (!entry.sinks) continue
        var outputId = entry.id.split("+input:")[0]
        var existing = byOutputId[outputId]
        if (!existing || entry.id.length < existing.id.length) byOutputId[outputId] = entry
      }

      var sinkPrefix = card.name.replace(/^alsa_card\./, "alsa_output.")
      for (var outId in byOutputId) {
        var chosen = byOutputId[outId]
        var suffix = outId.replace(/^output:/, "")
        var rawLabel = String(chosen.description || suffix).split(" + ")[0]
        var label = Model.friendlyDeviceLabel(rawLabel) || suffix
        var isHdmi = suffix.indexOf("hdmi") === 0
        options.push({
          label: isHdmi ? label : (suffix === "analog-stereo" ? "Speakers" : label),
          glyph: isHdmi ? "󰍹" : "󰓃",
          profile: outId,
          sinkSuffix: suffix,
    ...
    // Stable, sensible order: built-in analog/other first, HDMI ports after,
    // alphabetically within each group.
    options.sort(function(a, b) {
      var aHdmi = a.sinkSuffix.indexOf("hdmi") === 0 ? 1 : 0
      var bHdmi = b.sinkSuffix.indexOf("hdmi") === 0 ? 1 : 0
      if (aHdmi !== bHdmi) return aHdmi - bHdmi
      return a.label.localeCompare(b.label)
    })

    root.outputProfileOptions = options
    root.activeCardProfiles = activeMap
  }

  // Sink-name prefixes belonging to any card we found profiles for, so a
  // card's own sink doesn't also show up (duplicated) under "other devices".
  readonly property var builtinSinkPrefixes: {
    var seen = {}
    var list = []
    for (var i = 0; i < outputProfileOptions.length; i++) {
      var prefix = outputProfileOptions[i].sinkPrefix
      if (prefix && !seen[prefix]) { seen[prefix] = true; list.push(prefix) }
    }
    return list
  }

  // Devices no discovered card profile covers — Bluetooth, USB, anything
  // PipeWire lists independent of any ALSA card's active profile.
  readonly property var otherOutputSinks: {
    var list = []
    for (var i = 0; i < displayAudioSinks.length; i++) {
      var n = displayAudioSinks[i]
      if (!n || !n.name) continue
      var name = String(n.name)
      var belongsToCard = false
      for (var j = 0; j < builtinSinkPrefixes.length; j++) {
        if (name.indexOf(builtinSinkPrefixes[j]) === 0) { belongsToCard = true; break }
      }
      if (!belongsToCard) list.push(n)
    }
    return list
  }

  function switchCardProfile(option) {
    root.pendingDefaultSinkName = option.sinkPrefix + "." + option.sinkSuffix
    Quickshell.execDetached(["pactl", "set-card-profile", option.cardName, option.profile])
    cardProfileTimer.restart()
    pendingDefaultSinkRetries = 0
    pendingDefaultSinkTimer.restart()
  }

  function tryClaimPendingDefaultSink() {
    if (!root.pendingDefaultSinkName) return
    for (var i = 0; i < root.candidateSinks.length; i++) {
      var n = root.candidateSinks[i]
      if (n && String(n.name) === root.pendingDefaultSinkName) {
        root.setDefaultSink(n)
        root.pendingDefaultSinkName = ""
        return
      }
    }
    // Not ready yet — try again shortly, a few times, then give up quietly.
    pendingDefaultSinkRetries += 1
    if (pendingDefaultSinkRetries < 6) pendingDefaultSinkTimer.restart()
    else { pendingDefaultSinkRetries = 0; root.pendingDefaultSinkName = "" }
  }

  function sectionCount(section) {
    if (section === "output") return root.outputProfileOptions.length + otherOutputSinks.length
  ...
  function activateCursor() {
    ...
    if (focusSection === "output") {
      ...
      var profileCount = root.outputProfileOptions.length
      if (selectedIndex < profileCount) {
        root.switchCardProfile(root.outputProfileOptions[selectedIndex])
        return
      }
      var otherSink = root.otherOutputSinks[selectedIndex - profileCount]
      if (otherSink) setDefaultSink(otherSink)
      return
    }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateOutputProfileOptions(text)
    }
  }
  ...
  Timer {
    ...
    interval: 600
    repeat: false
    onTriggered: if (!cardProfileProc.running) cardProfileProc.running = true
  }
  ...
  // Once the profile switch lands, PipeWire creates the new sink a moment
  // later. This grabs it and makes it the actual default the instant it
  // exists, so picking a device both activates its profile AND routes audio
  // to it — not just flips which profile shows as active.
  Timer {
    id: pendingDefaultSinkTimer
    interval: 400
    repeat: false
    onTriggered: root.tryClaimPendingDefaultSink()
  }
      ...
      ScrollView {
        ...
        Column {
          ...
          Column {
            ...
            // Speakers / HDMI — profile-backed rows, always shown.
            Repeater {
              model: root.outputProfileOptions
            ...
            // Anything else PipeWire lists independent of the card's active
            // profile (Bluetooth, USB, etc.).
            Repeater {
              model: root.otherOutputSinks
              ...
              SinkRow {
                ...
                width: panelColumn.width
                node: modelData
                rowIndex: root.outputProfileOptions.length + index
              }
            }
          }
  ...
  // Profile-backed output row (Speakers / HDMI). Same look as SinkRow, but
  // switches the ALSA card profile — and claims the resulting sink as
  // default once PipeWire creates it — instead of calling setDefaultSink
  ...
  component ProfileRow: CursorSurface {
    ...
    readonly property bool isActive: root.activeCardProfiles[option.cardName] === option.profile
    ...
    MouseArea {
      ...
      onClicked: root.switchCardProfile(profileRow.option)
    }
  }

  // Output device row — cursor target inside the "output" section, for
  // devices PipeWire lists independent of the card's own profiles
  // (Bluetooth, USB, etc.). Mouse hover updates the panel cursor at the
  // root; visuals come entirely from hasCursor/current via CursorSurface,
